...
public class GatewayRouter {
	...
	public RouteLocator gatewayRoutes(RouteLocatorBuilder builder) {
		return builder.routes()
				.route(r -> r.path("/api/v1/auth/*/signin", "/api/v1/auth/reissue").filters(f -> circuitBreaker(f, EUREKA_AUTH)).uri(EUREKA_AUTH))
				...
				.route(r -> r.path("/api/v2/members/**")
						.filters(f -> circuitBreaker(f.filter(jwtAuthentication.apply(FALSE)), EUREKA_MEMBER)).uri(EUREKA_MEMBER))
