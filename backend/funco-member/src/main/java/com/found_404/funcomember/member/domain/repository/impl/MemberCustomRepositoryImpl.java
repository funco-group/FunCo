...
public class MemberCustomRepositoryImpl implements MemberCustomRepository {
	...
	public MemberInfo findUserInfoByMemberId(Long loginMemberId, Long memberId) {
		JPAQuery<Long> subQuery = jpaQueryFactory.select(subscribe.count())
			.from(subscribe)
			.where(subscribe.fromMember.id.eq(loginMemberId),
				subscribe.toMember.id.eq(memberId),
				subscribe.expiredAt.after(LocalDateTime.now()));
		...
		return jpaQueryFactory
			.select(new QMemberInfo(member.id, member.nickname, member.profileUrl, member.introduction, member.cash,
				new CaseBuilder()
					.when(subQuery.eq(1L))
					.then(SUBSCRIBE.getValue())
					.when(member.portfolioStatus.stringValue().eq(PortfolioStatusType.PUBLIC.toString()))
					.then(PUBLIC.getValue())
					.otherwise(PRIVATE.getValue())
					.as("portfolioStatus"), member.portfolioPrice))
			.from(member)
